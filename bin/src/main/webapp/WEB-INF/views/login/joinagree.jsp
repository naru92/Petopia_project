	...
	<main>
		<div class="main">
			...
			<article class="one">
				...
				<section>
					<input type="checkbox" class="checkall" id="checkall"> <label
						for="checkall">약관 전체동의</label>
				</section>
				<section>
					<div class="allcheck">
						<input type="checkbox" class="check01" id="check01"
							name="must-agree"> <label for="check01"> [필수] 서비스
							이용약관 </label><br> <input type="checkbox" class="check02"
							id="check02" name="must-agree"> <label for="check02">
							[필수] 개인정보 처리방침 </label><br> <input type="checkbox" class="check03"
							id="check03" name="must-agree"> <label for="check03">
							[필수] 개인정보 수집/이용 동의 </label><br> <input type="checkbox"
							class="check04" id="check04" name="must-agree"> <label
							for="check04"> [필수] 만 14세 이상입니다. </label><br>
					</div>
					<div>
						<div class="info">
		...
		for (var i = 0; i < checkList.length; i++) {
			...
			function checkboxChecking() {
				if (this.checked) { // 클릭한 값이 체크되었을 때
					...
					count--; // 카운트 -
					allcheck.checked = false; // allcheck 버튼 역시 6이 될 수 없으므로 체크값을 없애준다.
				}
			}
			;
		};
